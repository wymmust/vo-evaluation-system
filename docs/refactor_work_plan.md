...
将当前平铺结构重构为按评估流程分层的三层架构，CLI 和 Web 作为并列入口统一在 `python -m voeval {eval|serve}` 下。
...
│   ├── __main__.py                    # 统一入口：python -m voeval {eval|serve}
│   ├── cli.py                         # eval 子命令实现
...
python -m voeval serve --port 8766                                  # Web 服务
```
...
- 删除 `web/` 目录
- 验证：`pytest` 全绿 + `python -m voeval serve` 启动正常 + 浏览器访问正常 + HTML 导出正常
